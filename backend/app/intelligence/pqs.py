...
def adaptive_thresholds(settings: Settings, prior: ClvSportStat | None) -> tuple[float, int]:
...
def score_pick(*, features: PickFeatures, settings: Settings, prior: ClvSportStat | None) -> PQSResult:
    ...
    if features.time_to_start_minutes < settings.min_minutes_to_start:
        return PQSResult(0.0, PickScoreDecision.DROP, "min_minutes_to_start", {})
    if features.price_dispersion > settings.max_price_dispersion:
    ...
    dispersion_score = _clamp(1.0 - (features.price_dispersion / settings.max_price_dispersion))
